...
import { X, ChevronLeft, ChevronRight, Zap, Music, Video, Download, Lock, Star, Play, Pause } from "lucide-react";
import { useAuthModal } from "@/App";

const ALL_CATS = ["animated","neon","horror","anime","vertical","interactive","minimal","grunge","overlay","alert","bundle","pack"];

// ── Studio demo video placed in /public/studio-demo.mp4
const STUDIO_VIDEO_URL = "/studio-demo.mp4";

// ── Studio showcase images (horror/animated category — matches studio output aesthetic)
const STUDIO_IMAGES = [
        ...
        <div className="absolute bottom-4 left-0 right-0 flex justify-center gap-2 px-4 overflow-x-auto z-[60]">
          {images.map((img, i) => (
            <button key={i} onClick={e => { e.stopPropagation(); setIndex(i); }} onContextMenu={e => e.preventDefault()}
              className={`w-12 h-16 flex-shrink-0 overflow-hidden border-2 transition-all protected-media ${i === index ? "border-red-500 opacity-100" : "border-red-900/30 opacity-50 hover:opacity-80"}`}>
              <div className="wm" />
...
// ── Studio Demo Strip: video FIRST, then studio output images ──
function StudioStrip() {
  ...
  return (
    <div className="flex gap-3 min-w-max px-4 items-stretch">
      {/* VIDEO CARD — first position */}
      <div onClick={toggleVideo}
        className="relative flex-shrink-0 w-28 h-36 border border-red-600/60 hover:border-red-500 transition-all group overflow-hidden bg-black cursor-pointer">
        ...
        {/* Badge */}
        <div className="absolute top-1.5 left-1.5 bg-red-700/95 border border-red-500/80 px-1.5 py-0.5 flex items-center gap-1 z-10">
          {playing
            ? <Pause className="w-2.5 h-2.5 text-white" />
            : <Play className="w-2.5 h-2.5 text-white" />}
          <span className="text-white text-[8px] font-bold uppercase tracking-wider">Demo</span>
        </div>
        {/* Bottom label */}
        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black via-black/60 to-transparent px-2 py-2 z-10">
          <span className="text-[8px] text-red-400 font-bold uppercase tracking-widest">Studio Preview</span>
        </div>
      </div>
      ...
      {STUDIO_IMAGES.filter(Boolean).map((img, i) => (
        <div key={i}
          className="w-24 h-36 flex-shrink-0 border border-red-900/20 hover:border-red-500/50 transition-all cursor-pointer group relative protected-media overflow-hidden"
                ...
                <motion.div key={cat} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
                  ...
                  <Link href={`/products?category=${cat}`}>
                    <div className="group relative border border-red-900/30 hover:border-red-600/60 transition-all duration-300 overflow-hidden cursor-pointer bg-card lava-pulse">
                      <div className="aspect-[3/4] overflow-hidden relative protected-media" onContextMenu={e => e.preventDefault()}>
                        <div className="wm" />
                      ...
                      <div className="grid grid-cols-3 gap-0.5 border-t border-red-900/20">
                        {thumbs.map((t, ti) => (
                          <div key={ti} className="aspect-square overflow-hidden protected-media" onContextMenu={e => e.preventDefault()}>
                            <div className="wm" />
            ...
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
              <div className="space-y-6 mb-10">
                {[
                  ...
                  { icon: <Music className="w-5 h-5 text-red-500" />, title: "Sound Library & TTS", desc: "Add ambient horror sounds, screams, effects, or text-to-speech voices directly to your animation. 100+ sounds included." },
                  ...
                  { icon: <Download className="w-5 h-5 text-red-500" />, title: "Export in Multiple Formats", desc: "Record and download in 1080p, 4K, TikTok vertical, YouTube Shorts, OBS Canvas and more — all in-browser." },
                ].map((feat, i) => (
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.2 }}
              ...
              <div className="grid grid-cols-3 gap-2">
                <div className="col-span-2 row-span-2 relative overflow-hidden border border-red-900/40 group">
                  <img src={STUDIO_IMAGES[0]} alt="Studio output" draggable={false} onContextMenu={e => e.preventDefault()}
                    className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700"
                    style={{ minHeight: "300px" }} />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
                  <div className="absolute bottom-3 left-3">
                    <span className="text-xs text-red-400 uppercase tracking-widest bg-black/70 px-2 py-1 border border-red-900/30">🎬 Studio Output</span>
                  </div>
                </div>
          ...
          <div id="studio-plans">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-3xl mx-auto">
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }}
                ...
                <p className="text-gray-500 text-sm mb-6">Perfect to explore the studio and preview your creations</p>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }}
                ...
                <p className="text-gray-400 text-sm mb-6">Full power to record, download and publish your horror animations</p>
                <ul className="space-y-3 mb-8">
                  {["Everything in Free", "Record & Download videos", "1080p & 4K export", "TikTok, YouTube Shorts, OBS formats", "Unlimited projects", "Priority support"].map((text, i) => (
                    <li key={i} className="flex items-center gap-3 text-sm">
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }}>
      <Link href={`/products/${product.id}`}>
        <div className="group border border-red-900/30 bg-card hover:border-red-600/60 lava-pulse transition-all duration-300 overflow-hidden cursor-pointer">
          <div className="aspect-[3/4] overflow-hidden protected-media" onContextMenu={e => e.preventDefault()}>
            <div className="wm" />
