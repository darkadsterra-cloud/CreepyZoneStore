...
import SoundLibrary, { sharedAudioCtx, sharedAudioDest } from '@/components/SoundLibrary';
...
function removeProjectVideo(pid: string, vid: string) {
  ...
  videoStore[vid] = videoStore[pid].filter(v => v.id !== vid);
}
...
function VideoPlayer({ video, onClose }: { video: ProjectVideo; onClose: () => void }) {
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,0.96)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
      <div onClick={e => e.stopPropagation()} style={{ background: '#18181b', border: '1px solid #3f3f46', borderRadius: '12px', overflow: 'hidden', width: '100%', maxWidth: '780px', boxShadow: '0 25px 50px rgba(0,0,0,0.8)', margin: 'auto' }}>
        <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-800">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => { const a = document.createElement('a'); a.href = url; a.download = video.name; document.body.appendChild(a); a.click(); document.body.removeChild(a); }} className="flex items-center gap-1.5 px-3 py-1.5 bg-red-700 hover:bg-red-600 text-white text-xs font-bold rounded border border-red-500 transition-all">
        ...
        <div className="relative bg-black" style={{ lineHeight: 0 }}>
          <video ref={videoRef} src={url} onClick={togglePlay} onEnded={() => setPlaying(false)}
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 to-transparent p-3">
            <div className="w-full h-1.5 bg-zinc-700 rounded mb-2 cursor-pointer" onClick={e => { if (!videoRef.current) return; const r = e.currentTarget.getBoundingClientRect(); videoRef.current.currentTime = ((e.clientX - r.left) / r.width) * videoRef.current.duration; }}>
              <div className="h-full bg-red-500 rounded transition-all" style={{ width: `${progress}%` }} />
            </div>
            <div className="flex items-center gap-3">
              <button onClick={togglePlay} className="w-8 h-8 bg-red-700 hover:bg-red-600 rounded-full flex items-center justify-center text-white transition-all">
...
function ProjectFolderPopup({ currentProject, projects, onOpen, onNew, onClose, onGoLibrary }: {
  currentProject: HorrorProject | null; projects: HorrorProject[];
  onOpen: (p: HorrorProject) => void; onNew: () => void; onClose: () => void; onGoLibrary: () => void;
}) {
                    ...
                    {videos.map(v => (
                      <div key={v.id} className="bg-zinc-800/60 rounded-lg border border-zinc-700/30 overflow-hidden">
                        <div className="w-full h-14 bg-zinc-900 relative cursor-pointer" onClick={() => setPlayingVideo(v)}>
                          ...
                          <div className="absolute inset-0 flex items-center justify-center"><div className="w-8 h-8 bg-red-700/90 rounded-full flex items-center justify-center hover:bg-red-600"><Play className="w-4 h-4 text-white ml-0.5" /></div></div>
                        </div>
                        <div className="flex items-center gap-2 p-2">
                          <div className="flex-1 min-w-0"><p className="text-[9px] text-zinc-400 truncate">{v.name}</p><p className="text-[8px] text-zinc-600">{formatDur(v.duration)} · {formatSize(v.size)}</p></div>
                          ...
                          <button onClick={() => { const url = URL.createObjectURL(v.blob); const a = document.createElement('a'); a.href = url; a.download = v.name; document.body.appendChild(a); a.click(); document.body.removeChild(a); setTimeout(() => URL.revokeObjectURL(url), 5000); }} className="flex items-center gap-1 px-1.5 py-1 bg-red-700 hover:bg-red-600 text-white text-[8px] rounded font-bold"><Download className="w-2 h-2" /> DL</button>
      ...
      <div className="relative z-10 flex flex-col min-h-full">
        <div className="flex items-center justify-between px-4 md:px-8 py-4 border-b border-red-900/30 bg-zinc-900/80 sticky top-0 z-20">
          <div className="flex items-center gap-3">
            ...
            <div><h1 className="text-sm font-bold tracking-wider text-white" style={{ fontFamily: 'Georgia, serif' }}>Horror Animation Studio</h1><p className="text-[9px] text-zinc-500 tracking-widest uppercase">Project Library</p></div>
          </div>
        ...
        <div className="flex-1 px-4 md:px-8 py-6">
          <div className="max-w-4xl mx-auto">
            <div className="mb-6">
              {!showInput ? (
                <button onClick={() => setShowInput(true)} className="flex items-center gap-2 px-5 py-3 bg-red-700 hover:bg-red-600 text-white font-bold uppercase tracking-widest text-sm border border-red-500 transition-all w-full sm:w-auto justify-center"><Plus className="w-4 h-4" /> New Project</button>
              ) : (
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                  <input autoFocus value={newName} onChange={e => setNewName(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') handleNew(); if (e.key === 'Escape') setShowInput(false); }} placeholder="Project name..." className="px-4 py-2.5 bg-zinc-800 border border-red-700/50 text-white text-sm outline-none focus:border-red-500 flex-1" />
                    ...
                    return (
                      <div key={p.id} className="border border-zinc-800 bg-zinc-900/50 hover:border-red-700/50 transition-all group relative">
                        <div onClick={() => onOpen(p)} className="cursor-pointer p-4">
                          {p.thumbnail ? <div className="w-full h-28 overflow-hidden mb-3 bg-zinc-800 rounded"><img src={p.thumbnail} alt="" className="w-full h-full object-cover" /></div> : <div className="w-full h-28 bg-zinc-800/50 flex items-center justify-center mb-3 border border-zinc-700/30 rounded"><Skull className="w-10 h-10 text-zinc-700" /></div>}
                          <div className="flex items-start justify-between">
                            <div className="flex-1 min-w-0">
                              <h3 className="text-white font-bold text-sm truncate group-hover:text-red-300">{p.name}</h3>
                              <div className="flex items-center gap-2 mt-1 flex-wrap">
                                {p.status === 'finished' ? <span className="flex items-center gap-1 text-[9px] text-green-400 border border-green-800/40 px-1.5 py-0.5 rounded"><CheckCircle className="w-2.5 h-2.5" /> Finished</span> : <span className="flex items-center gap-1 text-[9px] text-yellow-500 border border-yellow-800/40 px-1.5 py-0.5 rounded"><Clock className="w-2.5 h-2.5" /> Draft</span>}
                                ...
                                {videos.map(v => (
                                  <div key={v.id} className="bg-zinc-800/60 rounded-lg border border-zinc-700/30 overflow-hidden">
                                    ...
                                    <div className="flex items-center gap-2 p-2">
                                      ...
                                      <button onClick={e => { e.stopPropagation(); const url = URL.createObjectURL(v.blob); const a = document.createElement('a'); a.href = url; a.download = v.name; document.body.appendChild(a); a.click(); document.body.removeChild(a); setTimeout(() => URL.revokeObjectURL(url), 5000); }} className="flex items-center gap-1 px-2 py-1 bg-red-700 hover:bg-red-600 text-white text-[9px] rounded flex-shrink-0 font-bold"><Download className="w-2.5 h-2.5" /> DL</button>
                                    </div>
                                  </div>
                                ))}
                              </div>
...
export default function HorrorStudio() {
  ...
  // Live refs — no stale closures in recording loop
  ...
  // ══════════════════════════════════════════════════
  // FIXED RECORDING ENGINE
  // FIXES:
  // 1. AUDIO — captures Web Audio API destination stream (sharedAudioDest)
  // 2. ANIMATIONS — canvas loop uses imagesRef which has live animation data
  // 3. FORMAT — saves as .webm (browser native) but named .mp4 for compatibility
  // ══════════════════════════════════════════════════
  const startRecording = useCallback(async () => {
    ...
    // ── AUDIO: Use shared Web Audio destination (captures all playing sounds) ──
    const videoStream = recCanvas.captureStream(30);
    const audioTracks: MediaStreamTrack[] = [];

    // Resume AudioContext if suspended
    if (sharedAudioCtx.current && sharedAudioCtx.current.state === 'suspended') {
      await sharedAudioCtx.current.resume();
    }

    // Get audio stream from shared Web Audio destination
    if (sharedAudioDest.current) {
      const audioStream = sharedAudioDest.current.stream;
      audioStream.getAudioTracks().forEach(t => audioTracks.push(t));
    }
    ...
    // Best quality WebM (widest browser support)
    const mimeTypes = [
    ...
    recorder.onstop = async () => {
      ...
      if (currentProject) {
        setSavingVideo(true);
        // Save as .mp4 extension (content is webm but plays in most players)
    ...
    // ── FRAME DRAWING LOOP — Full animation support ──
    const drawFrame = () => {
      const fc = ++frameCountRef.current;
      const imgs = imagesRef.current; // always latest
      ...
      const t = fc / 30; // time in seconds at 30fps
      ...
      // Draw each image WITH animations
      for (const img of drawList) {
        ...
        // Get animations — check both arrays
        const anims = (img.animations && img.animations.length > 0)
          ? img.animations
        ...
        const maxW = outWidth * 0.6;
        const maxH = outHeight * 0.6;
        ...
        for (const anim of anims) {
          ...
          if (a.includes('pulse') || a.includes('throb')) {
          ...
          if (a.includes('fade') || a.includes('ghost')) {
            animAlpha *= 0.5 + Math.sin(t * 1.5) * 0.4;
          }
          if (a.includes('swing')) {
            animRot += Math.sin(t * 2) * 0.3;
          }
          if (a.includes('creep') || a.includes('crawl')) {
            offX += Math.sin(t * 0.3) * (outWidth * 0.015);
            offY += Math.cos(t * 0.4) * (outHeight * 0.01);
          }
        }
        ...
        ctx.globalAlpha = Math.max(0, Math.min(1, (img.opacity ?? 1) * animAlpha));
    ...
    return (
      <div style={sizeStyle} className="relative overflow-hidden rounded-lg shadow-2xl shadow-black/80 border border-zinc-800">
        ...
        <div ref={isFullscreen ? undefined : previewRef} className={`w-full h-full relative overflow-hidden ${greenScreen ? 'bg-[#00ff00]' : 'bg-zinc-950'}`}>
          {!greenScreen && (
            <div className="absolute inset-0 pointer-events-none opacity-30" style={{ backgroundImage: 'linear-gradient(rgba(255,255,255,0.03) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.03) 1px, transparent 1px)', backgroundSize: '10% 10%' }} />
          )}
          {previewImages.map(img => (
            <div key={img.id} className={`absolute ${getAnimClass(img)}`} style={{ left: `${img.position.x}%`, top: `${img.position.y}%`, transform: `translate(-50%, -50%) scale(${img.scale}) rotate(${img.rotation}deg)`, opacity: img.opacity, zIndex: 5 }}>
              <img src={imageBlobStore[img.id] || img.url} alt={img.name} draggable={false} crossOrigin="anonymous" style={{ maxWidth: isFullscreen ? '450px' : '180px', maxHeight: isFullscreen ? '450px' : '180px', objectFit: 'contain', display: 'block' }} />
            </div>
          ))}
          ...
          <div ref={projectDropdownRef} className="relative">
            ...
            {showProjectDropdown && (
              <ProjectFolderPopup
                ...
                onOpen={handleOpenProject} onNew={() => setShowNewProjectModal(true)}
        ...
        <div className="w-[220px] flex-shrink-0 flex flex-col bg-zinc-900/80 border-r border-zinc-800 overflow-hidden">
              ...
              {images.map(img => {
                ...
                return (
                  <div key={img.id} onClick={() => setSelectedId(img.id)}
                    ...
                    <div className="w-8 h-8 rounded overflow-hidden flex-shrink-0 bg-zinc-800"><img src={imageBlobStore[img.id] || img.url} alt={img.name} className="w-full h-full object-cover" /></div>
                    <div className="flex-1 min-w-0">
          ...
          <div className="flex-1 overflow-y-auto p-3">
            {activeTab === 'animations' && (
              <AnimationPanel selectedAnimations={selectedImage?.animations ?? (selectedImage?.animation ? [selectedImage.animation] : [])} onToggle={toggleAnimation} onClearAll={clearAllAnimations} disabled={!selectedId} />
            )}
            {activeTab === 'sounds' && (
        ...
        <div className="flex-1 flex flex-col bg-zinc-950 min-w-0">
          <div className="flex items-center justify-between px-4 py-2 border-b border-zinc-800 flex-shrink-0 bg-zinc-900/50 backdrop-blur-sm">
            <div className="flex items-center gap-2">
              ...
              {recording && <span className="text-[9px] text-red-400 animate-pulse font-bold">● RECORDING → {ratio.width}×{ratio.height}</span>}
            </div>
            <div className="flex items-center gap-1.5">
              <button onClick={handleDownload} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-[11px] text-zinc-300 border border-zinc-700"><Download className="w-3 h-3" /> PNG</button>
              ...
              <button onClick={() => setFullscreen(true)} className="p-1.5 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-zinc-400 border border-zinc-700"><Maximize2 className="w-3 h-3" /></button>
            </div>
          </div>
